function plot_mg(warden, wdx, ctr, radius, r_w, idx)
    k = size(ctr, 1);
    m = size(idx, 1);
    w = size(wdx, 1);
    ...
    % Plot GUs not around Willie
    for i = 1:m
        plot(idx(i, 1), idx(i, 2), 'color', 'k', 'Marker', 'o', 'Markerfacecolor', 'k', 'MarkerSize', 3);
    end
    ...
    set(gcf, 'Position', [pos(1), pos(2) - 1, 6, 6]);
    xlim([-1000 1000]);
    ylim([-1000 1000]);
