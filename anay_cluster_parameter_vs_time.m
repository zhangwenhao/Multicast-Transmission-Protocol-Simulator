...
parfor index=0:looptimes
    cluster_parameter=index/looptimes;
    disp(index);
    loop_times(index+1)=cluster_parameter;
...
figure(1); 
plot(loop_times,POINT_TIME,'Color','k','LineStyle','--','Marker','*','MarkerFaceColor','none','MarkerSize',6,'MarkerIndices',1:1:length(loop_times),'linewidth',1);
hold on;
plot(loop_times,UNIFORM_TIME,'Color','k','LineStyle',':','Marker','square','MarkerFaceColor','none','MarkerSize',6,'MarkerIndices',1:1:length(loop_times),'linewidth',1);
...
xlabel('Cluster parameter, $\rho$','Interpreter','latex');
