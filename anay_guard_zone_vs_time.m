...
looptimes=20;
POINT_TIME=zeros(1, looptimes+1, 'double');
...
parfor index=0:looptimes
    %rng(index);
    r_w=index/looptimes*1000;
    disp(index);
    loop_times(index+1)=r_w;
    ...
    for d_l=1:distributionLoop
        ...
        [data, ~]=poisson_cluster(numberNodes,centerszie,cluster_parameter,radius); 
        k_p = size(data, 1);
     ...
     POINT_TIME(index+1) = distribution_Time_p/distributionLoop;
...
figure(1); 
plot(loop_times,POINT_TIME,'Color','k','LineStyle','--','Marker','*','MarkerFaceColor','none','MarkerSize',6,'MarkerIndices',1:2:length(loop_times),'linewidth',1);
hold on;

plot(loop_times,UNIFORM_TIME,'Color','k','LineStyle',':','Marker','square','MarkerFaceColor','none','MarkerSize',6,'MarkerIndices',1:2:length(loop_times),'linewidth',1);
plot(loop_times,MG_TIME,'Color','k','LineStyle','-','Marker','o','MarkerFaceColor','none','MarkerSize',6,'MarkerIndices',1:2:length(loop_times),'linewidth',1);

xlabel('Radius of guard zone, $r_w (m)$','Interpreter','latex');
